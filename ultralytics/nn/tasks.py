...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (FocusSPDConv, MESGCA, CoordAttnBlock, Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, DSConv, Focus,
                 ...
                 C2f_LWGA, C2f_CSSC, C2f_CNCM, C2f_HFRB, C2f_EVA):
            if args[0] == 'head_channel':
                args[0] = d[args[0]]
            c1, c2 = ch[f], args[0]
            ...
            if m in (VoVGSCSPC, BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, RepC3, DWRC3, C3_DWR, C2f_DWR, C3_DCNv2_Dynamic, C2f_DCNv2_Dynamic,
        ...
        # --------------ASF--------------
            ...
            c2 = ch[f] * 4
        elif m in {LearnableWaveletPool}:
            c2 = ch[f]
        elif m in {WaveletUnPool}:
            c2 = ch[f] // 4
        elif m in {CSPHyperOK}:
            c2 = ch[f]
            args = [c2]
